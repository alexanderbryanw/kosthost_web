...
//2. Query SQL
$sql = "SELECT  penyewaan.*,
                kost.namaKost,
                penghuni.namaPenghuni,
                penghuni.telepon
        FROM penyewaan
        JOIN kost
        ON penyewaan.IDKost = kost.IDKost
        JOIN penghuni
        ON penyewaan.IDPenghuni = penghuni.IDPenghuni
            ...
            ?>
                <div style="margin-top: 20px;">
                    <div class="card" style="width:250px">
                        <div class="card-body">
                            <h4 class="card-title" style="font-family: 'Playfair', serif;font-weight:bold"><?= $row['namaKost'] ?></h4>
                            <p class="card-text"><?= $row['namaPenghuni'] ?> <br> <?= $row['telepon'] ?></p>
                            <a href="viewdatapenghuni-pemilik.php?id=<?= $row['IDPenghuni'] ?>" class="btn btn-info">Cek</a>
                            <a href="../process/delete-penyewaan.php?id=<?= $row['IDSewa'] ?>" class="btn btn-danger">Hapus</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
